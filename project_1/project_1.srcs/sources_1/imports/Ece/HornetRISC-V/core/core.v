...
`ifndef SYNTHESIS
    always @(posedge clk_i) begin
        if (pc_o >= 32'h0000_7610 && pc_o <= 32'h0000_7620) begin
             $display("[CORE DEBUG] PC=%h | stall_IF=%b | fpu_stall_EX=%b fpu_start=%b fpu_done=%b | instr_i=%h",
                pc_o, stall_IF, fpu_stall_EX, fpu_start, fpu_done_EX, instr_i);
        end
    end
`endif
