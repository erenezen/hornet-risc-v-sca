...
begin
	if(!rst) begin
		...
		if(state == S1 && uart_rx_irq && uart_rx_byte == 8'h70) begin
		    reset_o <= 1'b0;
            reset_cnt <= 8'hFF; // Reset sayacını başlat
        end else if(state == S4 && counter == 2*SYS_CLK_FREQ) begin
		    reset_o <= 1'b0;
            reset_cnt <= 8'hFF;
        end else if (reset_cnt > 0) begin
            reset_o <= 1'b0; // Sayaç bitene kadar reset'i tut
